...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 text-gray-800 dark:text-gray-200 font-sans animate-fade-in">
      <nav className="bg-white/70 dark:bg-gray-800/70 backdrop-blur-xl sticky top-0 z-40 border-b border-gray-200/50 dark:border-gray-700/50 shadow-sm transition-all duration-300">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center gap-6">
              <div className="flex-shrink-0 transition-transform hover:scale-105 active:scale-95 duration-200">
                    ...
                    <button
                      ...
                      className={`flex items-center px-4 py-2 rounded-full text-sm font-medium transition-all duration-300 ${
                        activeView === item.view
                          ? 'bg-sky-100/80 text-sky-700 shadow-sm dark:bg-sky-900/40 dark:text-sky-300 ring-1 ring-sky-200 dark:ring-sky-800'
                          : 'text-gray-500 hover:bg-gray-100/50 hover:text-gray-900 dark:text-gray-400 dark:hover:bg-gray-700/50 dark:hover:text-white'
                      }`}
                      style={{ animationDelay: `${idx * 50}ms` }}
                    >
                      <item.icon className={`h-4 w-4 mr-2 ${activeView === item.view ? 'text-sky-600 dark:text-sky-400' : 'text-gray-400 dark:text-gray-500 group-hover:text-gray-500'}`} />
                      {item.label}
                      {item.count !== undefined && (
                        <span className={`ml-2 px-1.5 py-0.5 rounded-md text-[10px] font-bold ${
                          activeView === item.view
                            ? 'bg-white/50 text-sky-700 dark:bg-sky-950 dark:text-sky-300'
            ...
            <div className="flex items-center gap-4">
                ...
                <button onClick={logout} className="p-2 rounded-full text-gray-400 hover:text-white hover:bg-red-500 dark:hover:bg-red-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 transition-all duration-200 shadow-sm active:scale-90">
      ...
      <header className="bg-white/40 dark:bg-gray-800/40 backdrop-blur-sm shadow-sm border-b border-gray-100 dark:border-gray-700/50">
        <div className="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row justify-between items-center gap-4 animate-slide-up">
          <h1 className="text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-slate-800 to-slate-600 dark:from-white dark:to-slate-300 capitalize tracking-tight flex items-center">
            {activeView}
             <span className="ml-3 px-3 py-1 rounded-full bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-300 text-xs font-bold uppercase tracking-wider hidden sm:inline-block border border-slate-200 dark:border-slate-600">
                Overview
             </span>
          </h1>
             ...
             <div className="flex items-center space-x-3 flex-wrap justify-center md:justify-end">
                {user.role !== 'user' && (
                    ...
                    {activeView === 'allocation' && (
                       <button onClick={handleOpenAddCarModal} className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-xl shadow-sm text-white bg-sky-600 hover:bg-sky-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-sky-500 transition-all hover:-translate-y-0.5">
                          <PlusIcon/> <span className="ml-2">เพิ่มรถใหม่</span>
                       </button>
                    )}
                    {activeView === 'allocation' && (
                       <button onClick={() => setIsImportModalOpen(true)} className="inline-flex items-center px-4 py-2 border border-gray-300 dark:border-gray-600 text-sm font-medium rounded-xl shadow-sm text-gray-700 dark:text-gray-200 bg-white dark:bg-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-sky-500 transition-all hover:-translate-y-0.5">
                           <ClipboardPlusIcon /> <span className="ml-2">นำเข้า Excel</span>
                       </button>
                    )}
                     {activeView === 'stock' && (
                         <button onClick={() => setIsAddFromAllocationModalOpen(true)} className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-xl shadow-sm text-white bg-sky-600 hover:bg-sky-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-sky-500 transition-all hover:-translate-y-0.5">
                           <PlusIcon/> <span className="ml-2">เพิ่มรถเข้าสต็อก</span>
                       </button>
                    )}
                     {activeView === 'matching' && (
                         <button onClick={handleOpenAddMatchModal} className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-xl shadow-sm text-white bg-sky-600 hover:bg-sky-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-sky-500 transition-all hover:-translate-y-0.5">
                           <PlusIcon/> <span className="ml-2">เพิ่มรายการจับคู่</span>
                       </button>
                    )}
                    </>
                )}
                 <button onClick={() => setIsFilterVisible(!isFilterVisible)} className={`inline-flex items-center px-4 py-2 border text-sm font-medium rounded-xl shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-sky-500 transition-all hover:-translate-y-0.5 ${isFilterVisible ? 'bg-sky-50 dark:bg-sky-900/30 text-sky-700 dark:text-sky-300 border-sky-200 dark:border-sky-800' : 'border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-200 bg-white dark:bg-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600'}`}>
      ...
      {isFilterVisible && ['allocation', 'stock', 'matching', 'sold'].includes(activeView) && (
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 animate-scale-in">
              <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-md p-6 rounded-3xl shadow-lg border border-gray-100 dark:border-gray-700/50">
                   <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
                      <div className="md:col-span-3 lg:col-span-4">
                          ...
                          <input type="text" id="searchTerm" value={stagedFilters.searchTerm} onChange={e => handleFilterChange('searchTerm', e.target.value)}
                                 className="block w-full border border-gray-300 dark:border-gray-600 rounded-xl shadow-sm py-3 px-4 focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-transparent sm:text-sm bg-gray-50 dark:bg-gray-700/50 text-gray-900 dark:text-white transition-shadow"
                      ...
                      <div className="md:col-span-3 lg:col-span-4 grid grid-cols-1 md:grid-cols-2 gap-6 border-t border-gray-100 dark:border-gray-700 pt-6 mt-2">
                          <div>
                              ...
                              <input type="date" id="startDate" name="startDate" value={stagedFilters.startDate} onChange={e => handleFilterChange('startDate', e.target.value)}
                                      className="block w-full border border-gray-300 dark:border-gray-600 rounded-xl shadow-sm py-2 px-3 focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-transparent sm:text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
                              />
                          </div>
                          <div>
                   ...
                   <div className="mt-6 flex justify-end space-x-3 pt-4 border-t border-gray-100 dark:border-gray-700">
                       <button onClick={clearFilters} className="px-5 py-2.5 border border-gray-300 dark:border-gray-600 text-sm font-medium rounded-xl text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">Clear All</button>
                       <button onClick={applyFilters} className="px-5 py-2.5 border border-transparent text-sm font-medium rounded-xl text-white bg-sky-600 hover:bg-sky-700 shadow-md hover:shadow-lg transition-all">Apply Filters</button>
                   </div>
              </div>
          </div>
      )}
